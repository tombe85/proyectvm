...
public class RbfInstruction extends ParameterizedInstruction {

	/**
	 * RbfInstruction constructor
	 * @param arg
	 */
	public RbfInstruction(int arg) {
		super(arg);
	}
	...
	public Instruction parse(String insString) {
		...
		if(checkLine(insString) && extractInstructionString(insString).equalsIgnoreCase("rbf")){
			Integer argument = extractArgument(insString);
			if(argument != null)
				instruction = new RbfInstruction(argument);
		}
		return instruction;
	}
	...
	public String toString() {
		return "RBF " + getArgument().toString();
	}
